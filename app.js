...
const navIdeButton = document.getElementById("nav-ide");
const navDemoButton = document.getElementById("nav-demo");
...
function createGithubRepoWithCode() {
  const rawName = prompt("Repo adı", "arduino-blok-projem");
  if (!rawName) return;
  const name = sanitizeRepoName(rawName);
  if (!name) {
    window.alert("Geçerli bir repo adı girin. Sadece harf, rakam, nokta, alt çizgi ve tire desteklenir.");
    return;
  }
...
clearSerial.addEventListener("click", () => {
  serialBody.textContent = "> Seri monitör temizlendi.";
});
...
navHomeButton.addEventListener("click", leaveIde);
navIdeButton.addEventListener("click", enterIde);
navDemoButton.addEventListener("click", openDemo);
